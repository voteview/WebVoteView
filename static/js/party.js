...
var groupSel = "both", bothGroup, senateGroup, houseGroup, currSet, pmx,
	stateDimension, partycontroljson, clusterUpper, colourSet;
...
function tooltipText(d)
{
	...
	var chamberMapping =
		groupSel == "both" ? "House and Senate" : groupSel.toUpperCase();
	var result = `\
		<p>${getGetOrdinal(currCong)} Congress &gt; \
		<strong>${stateMap[d.key]}</strong></p> \
		${globalPartyName} control ${d.value}% of the \
		${chamberMapping} in this state.`;
	return(result);
}
...
{
	const partyName =
		party > 0 ? globalPartyName : "Congressional Median (Midpoint)";
...
q
    .await(function(
			error, pdat, cdat, partyname, stateboundaries, pcontrol,
			configFile, usaboundaries) {
		...
		else
		{
			...
			var partyname = {
				"partyname": `Party ${party_param}`,
				"fullName": `Party ${party_param}`,
				"pluralNoun": `Party ${party_param} Member`,
				"noun": `Party ${party_param}`};
		...
		var activeSet=0;
		var congressSet = cdat.filter(function(cong) {
			return +cong.congress >= min && +cong.congress <= max;
		});
		var memSetScatter = [];
		congressSet.forEach(function (d) {
			...
			if(party[0] !== undefined)
			{
				maxY = Math.max(
					maxY,
					d.congressmedian * 1.05,
					Math.max(...party[0].grandSet) * 1.05);
				minY = Math.min(
					minY,
					d.congressmedian * 1.05,
					Math.min(...party[0].grandSet) * 1.05);
			}
			else
			{
			...
			if(party[0] !== undefined)
			{
				...
				{
					memSetScatter.push(
						{"x": d.congress, "y": party[0].grandSet[qQ]});
				}
			}
		});
		...
		var congressDimMem = cfMemSet.dimension(function(d) {
			return [+d.x, +d.y];
		});
		var congressGroupMem = congressDimMem.group();
        var congressGroup = congressDimension.group().reduceSum(function (d) {
			return d.nMembers;
		});

        var dimParty = congressDimension.group().reduceSum(function (d) {
			return d.partymedian;
		});
        var dimCong = congressDimension.group().reduceSum(function (d) {
			return d.congressmedian;
		});

		// Set up the x-axis ticks. What we want is a tick every 20 years
		// (congresses ending in 6). Our last tick should be every 10
		// (congresses ending in 1) if necessary.
		var xAxisTickValues = [];
        ...
        timeChart
            ...
            .xAxis().tickValues(xAxisTickValues).tickFormat(function(v) {
				return (1787 + 2 * v) + 1; });
			...
			dimChart
				.compose([
					...
					dc.scatterPlot(dimChart)
						.group(congressGroupMem)
						.colors([partyCol[1]])
						.colorAccessor(colHack).symbolSize(3),
		        	dc.lineChart(dimChart)
						.group(dimParty)
						.colors([partyCol[0]])
						.defined(function(d) {
	...
	console.log("MaxCong = " + maxCong);
		...
		if(minCong > 10) {
			tickLabels.push(
				`1st Congress<br/><small>${congYear(1)[0]}-${congYear(1)[1]}\
				</small>`
			);
		}
		...
		if(minCong == maxCong && minCong > 1 && minCong < finalCong)
		{
			...
			tickLabels.push(
				`<small>${nameUse}<br/>Active ${congYear(minCong)[0]}-\
				${congYear(minCong)[1]}</small>`);
		}
		...
		else
		{
			...
			if(minCong > 1)
			{
				...
				else if(maxCong - minCong < 9)
					tickLabels.push("<small>Start of /</small>");
				else
					tickLabels.push(`<small>Start of<br/>${nameUse}</small>`);
			}
			...
			if(maxCong < finalCong)
			{
				...
				if(maxCong - minCong < 4) tickLabels.push(
					`<small>Start / End of<br/>${nameUse}</small>`);
		...
		tickPos.push(100);
		tickLabels.push(
			`${getGetOrdinal(finalCong)} Congress<br/>\
			<small>${congYear(finalCong)[0]}-${congYear(finalCong)[1]}\
			</small>`);
		...
		mapTopo = topojson.feature(
			stateboundaries,
			stateboundaries.objects.states).features;
		stateTopo = topojson.feature(usaboundaries,
			usaboundaries.objects.usa).features;
		partyMapChart
			.width(930)
			.height(500)
		...
		$.ajax({
			dataType: "JSON",
			url:
				`/api/getmembersbyparty?id=${party_param}&congress=\
				${initialCong}&api=Web_Party`,
	...
	function fadeStates(c)
	{
		...
		{
			if(currentYear[0] >= mapTopo[i].properties["STARTYEAR"] &&
				currentYear[1]<=mapTopo[i].properties["ENDYEAR"])
			{
				baseSVG.select("g.layer1")
	...
	function ensureLegend(c)
	{
		...
		legendBox.append("text")
			.attr("x", bX + 10)
			.attr("y", bY + 15 + ((colourSet.length) * 20))
			.attr("font-size", "0.9em")
	...
	function ensureTextLabel(c)
	{
		const congLabel =
			(groupSel == "both") ?
			"Congress" :
			groupSel.toUpperCase();
		const years = congYear(currCong);
		var textLabelTitle = `
			${getGetOrdinal(currCong)} ${congLabel} (${years[0]}-${years[1]})
		`;

		var baseSVG = c.svg();
		if(baseSVG.selectAll("g").filter(".textLabel")[0].length)
		{
			var textBox = baseSVG.selectAll("g")
				.filter(".textLabel")
				.selectAll("text");
	...
	function setupCongress(num) {
		...
		bothGroup = stateDimension.group().reduceSum(function(d) {
			return d["both"] });
		senateGroup = stateDimension.group().reduceSum(function(d) {
			return d["senate"] });
		houseGroup = stateDimension.group().reduceSum(function(d) {
			return d["house"] });
	...
	function switchCongress(num, autoLoop=0)
	{
		...
		if(slider.slider("getValue") != num) {
			slider.slider("setValue", parseInt(num)); }
		setupCongress(num);
	...
	function playLoopIteration()
	{
		...
		// Hang on the last, current congress before looping
		if(currCong == maxCong) { delay = 3000; }
		switchCongress(currCong, 1);

		// Loop on.
		if(!PREVENTLOOPBEHAVIOUR || currCong != maxCong)
