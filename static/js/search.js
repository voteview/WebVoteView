...
function exportJSON() {
	window.location = `/api/exportJSON?id=${cookieId}`;
}

function exportXLS() {
	window.location = `/api/download_excel?stash=${cookieId}`;
}
...
function updateStashCart()
{
	...
	if(mostRecentSearch.substr(0,6) == "saved:") $("#addAll").hide();
...
function toggleAdvancedSearch(instant)
{
	if(!instant)
	{
		...
		{
			$('#results-selects')
				.animate({width: 'toggle', opacity: 'toggle'}, 125, () => {
		...
		{
			$('#resultsHolder')
				.animate({width: '75%'}, 125, () => {
					$('#results-selects').animate(
						{ width: 'toggle', opacity: 'toggle' }, 125, 'linear'
					);
...
function startPulseSuggested()
{
	...
	$("#searchSuggest a").attr("href",
		`/search/${stripJunkFromSearch($("#searchSuggest a").html())}`
	);
}

var suggestions = [
	"john mccain", "tax congress: [100 to 112]",
	"support: [95 to 100]", "impeach chamber:Senate",
	"iraq war", "cuba", "france",
	"codes: Civil Liberties", "terrorism"];
var suggestedPulse;

$(document).ready(() => {
	$('[data-toggle="tooltip"]').tooltip();
	$('#stashCartBar').carousel(
		{ "interval": false, "keyboard": false, "wrap": false }
	);
	...
	// We do have a stash, so let's load the votes from it.
		...
		$.ajax({
			...
			success: (data, status, xhr) => {
				...
				updateStashCart();
				console.log(
					`Loaded votes: ${cachedVotes["old"].length} old and ${cachedVotes["votes"].length} new`
				);
				selectIncludedVotes();
			}
		});
	...
	$("#searchTextInput").on('input', () => {
		shouldSuggest = 0;
	});
	$("#searchTextInput").focus();
	$("#searchTextInput").on('focus', () => {
	...
	$(window).scroll(function () {
		...
		if ($(window).scrollTop() + $(window).height() >= $(document).height() * 0.95 &&
			nextId > 0 && metaPageloaded < 10 && !blockAutoscroll) {
			blockAutoscroll = 1;
			getRollcallsPage();
		}
	});
	...
	$("#faceted-search-form").submit((event) => {
	...
	if ($('#support').slider('getValue')[0] != 0 ||
		$('#support').slider('getValue')[1] != 100) {
...
function updateCheckAll(e)
{
	if(e.target.checked)
	{
		$('#faceted-search-form input[name="peltzman"], #faceted-search-form input[name="clausen"]')
			.each(function() {
