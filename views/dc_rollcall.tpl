...
<div class="container">
  <div class="row">
      <div class="col-md-12">
        <h3><abbr title="Chamber">{{ rollcall.chamber }}</abbr>/<abbr title="Session">{{ rcSuffix }} {{ rollcall.session }} Session</abbr>/<abbr title="Rollnumber">Vote {{ rollcall.rollnumber }}</abbr> </h3> 
        <p><strong>Date:</strong> {{ rollcall.date }}</p>
        <p><strong>Clausen Vote Category:</strong> {{ rollcall.code.Clausen.0 }} <br/> <strong>Peltzman Vote Category:</strong> {{ rollcall.code.Peltzman.0 }}</p>
...
(function loadData() {
    if (chamber == "House") {
        queue()
          .defer(d3.json, sprintf("/vv/static/json/districts%03d.json", {{ rollcall.session }}))
          .defer(d3.json, "/webvoteview/api/getvote/{{ rollcall.id }}")
          .await(drawWidgets);
    } else if (chamber == "Senate") {
        queue()
          .defer(d3.json, sprintf("/vv/static/json/states%03d.json", {{ rollcall.session }}))
          .defer(d3.json, "/webvoteview/api/getvote/{{ rollcall.id }}")
          .await(drawWidgets);
    }
})();
  ...
  if (false) {
    ...
    dc.dataTable(".dc-data-table")
		...
		//console.log(d);
            ...
            function (d) {
              return "<a class='btn btn-primary btn-sm' href='/webvoteview//person/" + d.id + "'>See profile</a>"
            }
        ]);
