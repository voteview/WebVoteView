...
def sort_votes_by_column(votes, sort_col, sort_dir):
    ...
    if sort_col == 2:  # Party Vote
        ...
        # Abstentions and votes without an estimated probability (lopsided
        # votes where NOMINATE skips the estimate) always sink to the bottom,
        # regardless of sort direction.
        sunk = [v for v in votes
                if v.get("myVote") == "Abs" or "myProb" not in v]
        with_prob = [v for v in votes
                     if v.get("myVote") != "Abs" and "myProb" in v]
        with_prob.sort(key=lambda v: int(round(v["myProb"])), reverse=reverse)
        votes = with_prob + sunk
